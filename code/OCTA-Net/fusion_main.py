...
sub_dir = f"{args.dataset}/second_stage_v2"

if isTraining:  # train
    NAME = f"{args.dataset}_second_stage_v2-2nd"
    viz = Visualizer(env=NAME)
    writer = SummaryWriter(f"{args.logs_dir}/{sub_dir}")
    mkdir(f"{args.models_dir}/{sub_dir}")
    ...
    first_net_thick = torch.load(
        f"{args.models_dir}/{args.dataset}/first_stage/front_model-{args.first_suffix}"
    ).to(device)
    first_net_thick = first_net_thick.module
    first_net_thick.eval()
    first_net_thin = torch.load(
        f"{args.models_dir}/{args.dataset}/first_stage/front_model-{args.first_suffix1}"
    ).to(device)
    ...
    for epoch in range(args.second_epochs):
        ...
        if (epoch + 1) % args.val_epoch_freq == 0 or epoch == args.second_epochs - 1:
            second_net, best_fusion = val_second_stage(
                best_fusion,
                viz,
                writer,
                val_dataloader,
                ...
                f"{args.models_dir}/{sub_dir}",
                f"{args.results_dir}/{sub_dir}",
                epoch + 800,
    ...
    first_net = torch.load(
        f"{args.models_dir}/{args.dataset}/first_stage/front_model-{args.first_suffix}"
    ).to(device)
    first_net = first_net.module
    first_net.eval()
    first_net1 = torch.load(
        f"{args.models_dir}/{args.dataset}/first_stage/front_model-{args.first_suffix1}"
    ).to(device)
    first_net1 = first_net1.module
    first_net1.eval()
    second_net = torch.load(
        f"{args.models_dir}/{sub_dir}/fusion_model-{args.second_suffix}"
    ).to(device)
    second_net.eval()
