...
sub_dir = f"{args.dataset}/first_stage"

if isTraining:  # train
    NAME = f"{args.dataset}_first_stage-2nd"
    ...
    for epoch in range(args.first_epochs):
        ...
        if (epoch + 1) % args.val_epoch_freq == 0 or epoch == args.first_epochs - 1:
            first_net, best_thin, best_thick, best_fusion = val_first_stage(
                best_thin,
                best_thick,
                best_fusion,
                viz,
    ...
    net = torch.load(
        f"{args.models_dir}/{sub_dir}/front_model-{args.first_suffix}"
    ).to(device)
    net.eval()
